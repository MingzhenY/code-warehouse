...
void Get(int L,int R,int &MaxV,int &MinC){
...
int GetMax(int L,int R){
	int ANS=0;
	for(int s=N+L-1,t=N+R+1;s^t^1;s>>=1,t>>=1){
		if(~s&1 && Max[s^1]>ANS) ANS=Max[s^1];
		if( t&1 && Max[t^1]>ANS) ANS=Max[t^1];
	}
	return ANS;
}
int GetMin(int L,int R){
	int ANS=10000000;
	for(int s=N+L-1,t=N+R+1;s^t^1;s>>=1,t>>=1){
		if(~s&1 && Min[s^1]<ANS) ANS=Min[s^1];
		if( t&1 && Min[t^1]<ANS) ANS=Min[t^1];
	}
	return ANS;
}
int n,k,x;
