		...
		switch(Dir){
			...
			case 1:
				for(int i=0;i<N;i++)
					for(int j=0;j<N;j++)
						for(int k=0;k<B.cell[i][j];k++)
								if(~cell[N-1-k][N-1-i][j]) T=0;	
				if(T){//如果无矛盾，将这个Block加入。 
					for(int i=0;i<N;i++)
						for(int j=0;j<N;j++)
							for(int k=0;k<B.cell[i][j];k++)
								cell[N-1-k][N-1-i][j]=Num;
				}
				break;
			case 2:
				for(int i=0;i<N;i++)
					for(int j=0;j<N;j++)
						for(int k=0;k<B.cell[i][j];k++)
							if(~cell[j][N-1-k][N-1-i]) T=0;	
				if(T){//如果无矛盾，将这个Block加入。 
					for(int i=0;i<N;i++)
						for(int j=0;j<N;j++)
							for(int k=0;k<B.cell[i][j];k++)
								cell[j][N-1-k][N-1-i]=Num;
				}
				break;
			case 3:
				for(int i=0;i<N;i++)
					for(int j=0;j<N;j++)
						for(int k=0;k<B.cell[i][j];k++)
							if(~cell[N-1-i][j][N-1-k]) T=0;	
				if(T){//如果无矛盾，将这个Block加入。 
					for(int i=0;i<N;i++)
						for(int j=0;j<N;j++)
							for(int k=0;k<B.cell[i][j];k++)
								cell[N-1-i][j][N-1-k]=Num;
				}
				break;
			case 4:
				for(int i=0;i<N;i++)
